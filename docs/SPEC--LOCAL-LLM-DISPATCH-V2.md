...
> **สถานะ:** DRAFT · 2026-07-03
> **แทนที่:** SPEC--LOCAL-MODEL-ANTI-ERROR-LOOP (ไฟล์เดิมสูญหาย — สเปกนี้ self-contained, รวมกติกาที่พิสูจน์แล้วทั้งหมด)
> **หลักฐานอ้างอิง:** [REPORT--LOCAL-LLM-DISPATCH.md](REPORT--LOCAL-LLM-DISPATCH.md) (dispatch จริง 3 ครั้ง, Wave 0–5) · [LOCAL_MODEL_LEDGER.md](LOCAL_MODEL_LEDGER.md)
...
| FR-5 | VRAM scheduler (mutex + re-warm) | qwen3 ~9GB ชน Demucs/whisper | สูงบนเครื่องนี้ |
...
```
  ...
  โมเดลใหม่ ──▶ FR-1     │  qwen3:latest (14.8B)  [default, known-good]│
  smoke-test ──pass──▶  │  sushirl:9b / Qwythos-9B [candidate]       │
       │                └──────────────┬───────────────────────────┘
...
ไฟล์ที่สเปกนี้กำหนด (อยู่ใต้ `D:\G-Music\orchestration\` — สร้างใหม่):
...
**กติกา core เดียวกับ Rwang:** ทุกสคริปต์เป็น **deterministic, stdlib-only** (ยกเว้น `recall_mistakes.py` เรียก Ollama embeddings ผ่าน HTTP — ยังไม่มี SDK) · ห้ามมี LLM SDK call · CLI/IO shape คงที่

---

## 3. FR-1 — Smoke-test suite (model onboarding gate)

**หลักฐาน:** gemma-4-12B-coder คืน `<unused30><unused14>` (eval_count = 4) — GGUF/chat-template เสีย, เสียเวลา 161.7s กลาง batch จริง ทั้งที่ตรวจได้ใน 3 นาทีก่อนใช้ · บทเรียน: **การเลือกโมเดลสำคัญกว่าการจูน prompt**
...
### 3.4 Acceptance ของ FR-1 เอง
- รัน `smoke_test.py qwen3:latest` → exit 0, JSON มี latency cold+warm ครบ
...
# stdout: {"model":"qwen3:latest","pass_rate":1.0,"n":7,"median_warm_latency_s":5.8,"reason":"best score, n>=5"}
```
กติกา (เรียงตามลำดับ):
...
- `num_predict ≥ 2000` **ยังบังคับ** — thinking token ของ qwen3 กินโควตาก่อนถึง JSON

### 5.2 เงื่อนไขเปิดใช้ (per-model, พิสูจน์ก่อนบังคับ)
structured output กับ thinking model อาจลดคุณภาพ code (โมเดลบางตัวเขียน code ใน string JSON ได้แย่กว่าใน fence) → **ต้อง A/B ผ่าน smoke-test ก่อน**:
...
- ไม่งั้นใช้ fence + strip ต่อไป (ผลจริงจาก report: fence + strip ใช้ได้ 2/2 อยู่แล้ว — อย่าเปลี่ยนถ้าไม่ดีขึ้นจริง)

### 5.3 Acceptance ของ FR-3
- dispatch จริง 1 งานกับ qwen3 โหมด json → gate pass และไม่มีการเรียก regex fallback
...
**หลักฐาน:** การ inject "avoid gemma-coder — ใช้ qwen3" 1 บรรทัดใน dispatch #3 ทำงานได้ดี แต่เป็น manual — พอ ledger โต การเลือกว่า mistake ไหน "เกี่ยว" กับ task ปัจจุบันจะไม่ scale
...
{"ts":"2026-07-02T…","kind":"fail","model":"hf.co/yuxinlu1/gemma-4-12B-coder…","task_id":"metronomeTicks",
 "task_type":"pure-math","severity":"critical","lesson":"GGUF/chat-template เสีย คืน special token — อย่าใช้โมเดลนี้ ให้ qwen3",
 "blacklist":true}
{"ts":"2026-07-03T…","kind":"pass","model":"qwen3:latest","task_id":"estimateSpeechDurationSec",
 "task_type":"format-util","lesson":"acceptance ที่เป็นเลขคำนวณได้ทำให้ output ตรงเป๊ะ","blacklist":false}
```
...
**หลักฐาน:** qwen3 14.8B กิน ~9GB · cold 186s vs warm 5.6s (~33x) · งาน ML หนัก (Demucs/whisper) ต้องการ VRAM ก้อนเดียวกัน
...
4. **ทางเลือก 9B co-resident** (sushirl:9b / Qwythos-9B ~6GB): เป็น candidate — ต้องผ่าน FR-1 smoke + FR-2 candidate flow ก่อน **ห้ามสละ qwen3 (known-good ตัวเดียว) เพื่อ VRAM จนกว่า 9B จะพิสูจน์คุณภาพ**
...
**หลักฐาน:** 2/2 ที่ผ่าน gate = pure function ไฟล์เดียว ไม่มี import + acceptance เป็นเลขคำนวณได้ · งาน stateful/integration ทั้งหมดไป Sonnet · ตรงกับ HARD RULE ของ `route.py` (ไม่มี verify_command → floor T2)
...
| E2 | pure: ไม่มี I/O, ไม่มี state, ไม่มี import | spec ของ task |
...
### 8.2 Taxonomy ชนิดงานที่อนุมัติ (ขยายจาก 2 → 5 ชนิด ตาม roadmap report §7.4)
...
|---|---|---|
| model | จาก `pick_model.py` (default ปัจจุบัน: `qwen3:latest`) | FR-2 |
| `temperature` | 0.1 | code deterministic (พิสูจน์แล้ว) |
| `num_ctx` | 8192 | micro-task + grounded context พอ |
| `num_predict` | ≥ 2000 | เผื่อ `<think>` ของ thinking model |
| `keep_alive` | `"30m"` | FR-5 |
| `format` | JSON schema เฉพาะโมเดลที่ A/B ผ่าน | FR-3 §5.2 |
| strip | `re.sub(r"<think>.*?</think>","",out,flags=re.S)` | fallback chain |
| extract | `re.search(r"```(?:ts|typescript)?\s*(.*?)```", out, re.S)` | fallback chain |
...
|---|---|---|
| **P1** | FR-1 smoke_test.py + smoke_tasks.json · รันกับ qwen3 + 9B candidates 2 ตัว | qwen3 pass บันทึกเป็น baseline; 9B ทั้งสองมีผล pass/fail ชัด |
...
> รูปแบบ spec YAML สำหรับให้ Rwang รันจริง (verify_command, tier_hint, depends_on ต่อ phase) — แปลงจากตารางนี้ตอนจะ execute; สเปกนี้เป็น SSOT เชิงเนื้อหา
...
|---|---|
| 9B ทั้งสองตัว smoke ไม่ผ่าน → ไม่มีทางออก VRAM นอกจาก mutex | FR-5 mutex ออกแบบให้พอใช้ได้โดยไม่มี 9B (ต้นทุน = 1 cold/สลับ) |
| structured output ลดคุณภาพ code ของ thinking model | FR-3 บังคับ A/B ก่อนเปิด; fence+strip เป็น default ที่พิสูจน์แล้ว |
| stats น้อยเกินไปช่วงแรก (n < 5 ทุกตัว) → router เลือกมั่ว | กติกา candidate จำกัด 1 งาน/batch + qwen3 เป็น default fallback เสมอ |
| Ollama อัปเวอร์ชัน/re-quantize ทำโมเดลเดิมเปลี่ยนพฤติกรรม | smoke_tasks.json คงที่ → รัน re-smoke หลังทุกอัป (regression) |
| ledger โตจน embed แพง | cache vector + re-embed เฉพาะ entry ใหม่ (§6.2) |

**คำถามเปิด (ตัดสินใจตอน execute):**
1. `orchestration/` scripts ควรอยู่ใน G-Music หรือย้ายเข้า Rwang core? — เอียงไปทาง G-Music ก่อน (target-specific) แล้ว port ขึ้น Rwang เมื่อ target ที่สองต้องใช้
2. threshold demote (0.6) และ score weight (0.1/10s) — ค่าตั้งต้นจากสามัญสำนึก ต้อง calibrate จากข้อมูลจริงหลัง P6
