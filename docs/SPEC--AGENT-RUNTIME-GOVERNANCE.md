...
| Access Scope = เพดานสิทธิ์ที่ spawn จริง (access-scope-enforcement) | — (code ล้วน — เพดานอยู่ที่ spawn args) | `engine.mjs permissionFor`: declared tier กด profile ลงตาม `config tierPermissions` (read/safe/shell/full) ไม่มีการยกขึ้น; guard = `access_scope_check.py` (structural) | `governance_lint.json` | ✅ GP8 smoke ที่ G-Maiden de1c6c94 (2026-07-10): argv จริง 5/5 — H2+code→acceptEdits, H0→plan, H3→+allowedTools Bash, legacy→เดิมเป๊ะ, H4+docs ไม่ถูกยก |
