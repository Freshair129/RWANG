...
> **ชั้นของเอกสาร:** governance layer — คุม **ทุก run** ของ orchestrator (Rwang, `G:/Rwang`) เหนือชั้น dispatch ([SPEC--LOCAL-LLM-DISPATCH-V2.md](SPEC--LOCAL-LLM-DISPATCH-V2.md))
...
|---|---|---|---|---|
| 1 | Context loss | **G1** External State Contract (ไฟล์บังคับ 5 ชนิด + restart protocol) | `state_check.py` + first-window protocol | 🆕 |
| 2 | State drift | **G2** Reconcile-before-act | `drift_check.py` ทุก phase boundary | 🆕 |
| 3 | Uncontrolled tool use | **G3** Action classification 4 ระดับ + hard gate | no-credential + hook/wrapper + `human_review` halt | บางส่วนมีแล้ว (Rwang invariant #1) |
| 4 | No deterministic coordination | **G4** Ownership declaration + waves + worktree isolation | `owners_check.py` ก่อนรัน + `run.js` topological waves | บางส่วนมีแล้ว |
| 5 | No auditability | **G5** Append-only event log ตอบ "5 คำถาม audit" ได้ | `progress.py` (writer เดียว) + schema ขยาย | มีแล้ว — ขยาย field |
| 6 | Prompt-only governance ไม่พอ | **G6** Governance Matrix + `governance_lint.py` (meta-guard) | lint ก่อนเริ่มทุก run | 🆕 หัวใจของเอกสาร |
| 7 | Long-horizon quality decay | **G7** Fresh-context verifier + holdout cases + verify-claims rule | gate รัน holdout + verifier subagent | 🆕 |

ไฟล์ guard ใหม่ทั้งหมดอยู่ `G:/Rwang/orchestrator/governance/` (เป็น policy ระดับ orchestrator ใช้ได้ทุก target — ดู open question §13) · กติกา core เดิม: **deterministic, stdlib-only, ห้ามมี LLM SDK**
...
> **กติกาเหล็ก:** แถวใดช่อง *runtime guard* ว่าง = policy นั้น**ยังไม่มีผลบังคับ** ห้าม claim ใน report/README ว่าระบบ "มี" policy นั้น · เพิ่ม policy ใหม่ = เพิ่มแถว + เขียน guard + acceptance test **ก่อน** ประกาศใช้
...
- **ตรวจ:** ทุก task ที่ `status: pass` ต้อง (ก) ไฟล์ที่อ้างใน `files[]` มีอยู่จริงและปรากฏใน `git diff <run-branch>` (ข) `verify_command` รันซ้ำแล้วยังผ่าน (cache ด้วย hash ของ diff — ไม่รันซ้ำถ้า diff ไม่เปลี่ยน)
...
- **วิ่งเมื่อ:** ก่อนเริ่มทุก run (`run.js` route phase เรียกก่อน route.py) — governance เสีย = ไม่เริ่มงาน
...
| Phase | ส่งมอบ | Definition of Done |
|---|---|---|
| **GP1** | `governance.yaml` + `governance_lint.py` + `test_guards.py` | ลบ guard 1 ตัว → lint fail → run เริ่มไม่ได้ |
| **GP2** | G1: ไฟล์บังคับ + `state_check.py` + restart_prompt.md | kill-restart test ผ่าน (agent สดกู้ state ตรง 100%) |
| **GP3** | G2: `drift_check.py` + wiring เข้า phase boundary ของ run.js | claim ปลอม → drift_detected event |
| **GP4** | G3: blocked_patterns + hook + confirm list | ทุก action class ให้ผลตามตาราง §5 |
...
| drift_check รัน verify ซ้ำ = แพง | cache ด้วย hash ของ diff — diff ไม่เปลี่ยนไม่รันซ้ำ (§4) |
...
**คำถามเปิด:**
1. guard scripts อยู่ Rwang (`orchestrator/governance/`) ตามร่างนี้ หรือเริ่มที่ target ก่อนแบบ dispatch v2? — เอียง Rwang เพราะเป็น policy ของ orchestrator ใช้ซ้ำได้ทุก target
2. PreToolUse hook ผูกกับ harness ไหน (Claude Code hooks vs wrapper script ใน run.js) — ต้องเลือกตอน GP4 ตาม runtime จริงที่ agent วิ่ง
