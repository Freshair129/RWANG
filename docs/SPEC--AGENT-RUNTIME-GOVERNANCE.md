...
| Access Scope = เพดานสิทธิ์ที่ spawn จริง (access-scope-enforcement, **planned GP8**) | — (code ล้วน — เพดานอยู่ที่ spawn args) | `engine.mjs permissionFor`: declared tier กด profile ลงตาม `config tierPermissions` (read/safe/shell/full) ไม่มีการยกขึ้น; guard = `access_scope_check.py` (structural) | `governance_lint.json` | atom ประกาศ H2 แต่ type=code → profile ถูกกดเหลือ safe (Bash ถูก block); flip เป็น enforced หลัง e2e smoke ใน runnable origin |
| engine daemon เคารพ meta-guard (engine-lint-interlock, **planned GP8**) | — (code ล้วน) | `engine.mjs governanceInterlock` ที่ boot (`server.mjs`) + `governanceBlock` ใน `runPool`/`dispatchOne` (cache ตาม mtime ของ matrix); guard = `engine_interlock_check.py` (structural) | `governance_lint.json` | matrix พัง → `node server.mjs` ปฏิเสธ dispatch เหมือน run.js ปฏิเสธ run; smoke ก่อน flip |
