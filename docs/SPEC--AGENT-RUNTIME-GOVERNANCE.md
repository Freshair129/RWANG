...
| Access Scope = เพดานสิทธิ์ที่ spawn จริง (access-scope-enforcement) | — (code ล้วน — เพดานอยู่ที่ spawn args) | `engine.mjs permissionFor`: declared tier กด profile ลงตาม `config tierPermissions` (read/safe/shell/full) ไม่มีการยกขึ้น; guard = `access_scope_check.py` (structural) | `governance_lint.json` | ✅ GP8 smoke ที่ G-Maiden de1c6c94 (2026-07-10): argv จริง 5/5 — H1+code เขียนได้, H0→bounded (ห้าม Glob/Grep/Bash), H2+code→acceptEdits, H3→+allowedTools Bash, legacy→เดิมเป๊ะ, H4+docs ไม่ถูกยก |
| W3 บังคับ lead review ห้ามข้าม (w3-lead-review) | — (code ล้วน) | `engine.mjs requireReviewFor`: `fanoutDegree>=6` บังคับ review=true เหนือทุก skip (global off / requireReview:false / skipForDraft) → task coupling 6-8 หลบ review ไม่ได้; guard = `w3_review_check.py` (structural, V2 เช็คว่า force วางก่อน skip) | `governance_lint.json` | ✅ GP12 smoke ที่ G-Maiden (2026-07-10): W3 deg6 +opt-out +ollama ยังบังคับ review; W2 deg2 opt-out เคารพเดิม — ตรงกับ G3.4 ที่ hop_metrics ชี้ |
| W4 super-hub ต้อง confirm ก่อน dispatch (w4-superhub-gate) | — (code ล้วน) | `engine.mjs`: `fanoutDegree` (undirected = deps ตัวเอง + task ที่ขึ้นกับมัน ตรงกับ hop_metrics) + `isW4`(>=9) เข้า `needsConfirm` → W4 ผ่านประตู confirm เดิม (approved=confirm, decomposed=ลด<9); guard = `w4_gate_check.py` (structural) | `governance_lint.json` | ✅ GP11 smoke ที่ G-Maiden (2026-07-10): H deg9 ถูกปฏิเสธ "W4 super-hub (degree 9 ≥ 9)"; M deg8 ผ่าน; confirm แล้วเคลียร์ — ตรงกับ `entity--atom-schema`/`feature--atom-store`/`G0.1` ที่ hop_metrics ชี้ |
| gate owner อ่านอย่างเดียว (role-readonly-gate) | — (code ล้วน) | `engine.mjs permissionFor(t,{role})`: role เป็น bound ที่สาม กดต่ำสุดเสมอ; `runReview` spawn ด้วย profile `read` (ถอด Edit/Write/MultiEdit/NotebookEdit/Bash); guard = `role_readonly_check.py` (structural) | `governance_lint.json` | ✅ GP9 smoke ที่ G-Maiden (2026-07-10): reviewer argv = `acceptEdits --disallowedTools Edit Write MultiEdit NotebookEdit Bash` (perm=read) ส่วน worker ยัง full — ปิดช่องที่ reviewer เคยแก้ชิ้นงานที่ตัวเองตัดสินได้ |
| engine daemon เคารพ meta-guard (engine-lint-interlock) | — (code ล้วน) | `engine.mjs governanceInterlock` ที่ boot (`server.mjs`) + `governanceBlock` ใน `runPool`/`dispatchOne` (cache ตาม mtime ของ matrix); guard = `engine_interlock_check.py` (structural) | `governance_lint.json` | ✅ GP8 smoke ที่ G-Maiden (2026-07-10): lint exit 1 → dispatchOne/runPool ปฏิเสธก่อน claim; lint เขียว → ทะลุไป confirm-gate โดยไม่ spawn; lint จริงข้าม repo 23.4s + cache ทำงาน |
