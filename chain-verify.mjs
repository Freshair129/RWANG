// chain-verify.mjs â€” read-only JS mirror of `orchestrator/progress.py verify-chain` (Rwang repo).
...
// DIFFERENTLY â€” a float always carries a decimal point (0.0 -> "0.0", 1.0 -> "1.0"),
...
// numeric field is a Python float â€” `cost_usd`; every other number (`attempt_id`,
...
// serializes as "0.0" while a true int (visible_exit 0) serializes as "0" â€” matching
 ...
 * run â€” zero hashed events, no snapshot tip â€” is also ok, per the documented rule).
 */
export function verifyChain(runDir) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (typeof stored !== "string" || !stored) {
      ...
      // else: legacy prefix (pre-chain writer) â€” allowed, chain not begun.
      continue;
    }
    ...
    if (recomputed !== stored) {
      errors.push(`line ${lineNo}: event_hash mismatch â€” event content was altered`);
    }
  ...
  if (nHashed === 0) {
    if (snapTip === null || snapTip === undefined) {
      ...
      // new-format run with zero events yet â€” an empty chain is intact
    } else {
      errors.push(
