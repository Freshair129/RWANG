// runs-reader.mjs — read-only reader over Rwang's runs/<runId>/ directory tree.
// Node ESM, zero-dependency (stdlib only, matches accounts.mjs / account-identity.mjs style).
// NEVER writes into runsDir — this module only reads progress.json / progress.ndjson.
//
// Schema reference: G:/Rwang/USERFLOW.md "The shared progress schema" — progress.json is the
...
// without throwing — a single broken run must never take down the whole list.
export function listRuns(runsDir) {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      // missing progress.json, unreadable dir, or malformed JSON — skip, don't throw
      continue;
    }
...
// Returns [] if the file is missing/unreadable — never throws.
