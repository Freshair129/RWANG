// accounts.mjs â€” multi-account registry + rotation for quota-spreading across plan/key accounts.
//
...
// an account just sets an env var before spawn â€”
...
// â”€â”€ registry: merge public provider.accounts[] with the gitignored secrets file â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ apply one account to a child env (the only spawn-time effect) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ pick the next usable account (skips cooling-down ones) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export function pickAccount(provider, pstate, { now = Date.now(), rotationOverride = null } = {}) {
  ...
  // "failover" (sticky â†’ preserve prompt cache); quota-heavy/parallel roles run "round-robin".
...
// â”€â”€ detect a usage-limit / rate-limit signal from a dispatch result â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ record the outcome of a dispatch onto the account state â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// "frees up in â€¦" countdown: when that event ages out at oldest+ms, its usage drops off).
...
// â”€â”€ file-backed state convenience (load â†’ mutate â†’ save) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export function selectAccount(providerName, provider, statePath, { now = Date.now(), rotationOverride = null, forceId = null } = {}) {
  ...
  // forceId pins a specific account (e.g. a pulse targeting one card) â€” bypasses rotation & cooldown.
...
// â”€â”€ live PLAN quota (only where the provider actually exposes one) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// The card should show the *subscribed plan's* quota, not Rwang's own dispatch tally. That's only
// readable for OpenRouter (GET /api/v1/key â†’ usage/limit/limit_remaining, in $). claude/codex/
// antigravity CLIs write no quota to disk and expose no endpoint, so there is nothing to read â€”
// their cards fall back to a Rwang-tracked estimate vs the cap you declare (clearly labelled).
// Returns a map keyed "<provider>/<id>" â†’ { source, used, limit, remaining, unit }.
// claude (Claude Code max/pro): the SAME endpoint Claude Code's own "Plan usage" panel reads â€”
// GET /api/oauth/usage with the login-dir's OAuth token â†’ real 5h/7d utilization + reset instants.
// This is what makes Rwang's claude card match Claude Code exactly (not an estimate). Token is a
// secret read from {configDir}/.credentials.json and NEVER returned â€” only the usage numbers are.
...
async function computePlanQuotas(config, secretsPath, fetchImpl) {
  ...
  if (or) {
    ...
    for (const a of or.accounts) {
      ...
      } catch { /* offline / bad key â†’ leave to the Rwang-tracked fallback */ }
    }
  }
  const cl = reg.claude;
  if (cl) {
...
// â”€â”€ read-only status view (CLI `accounts` + GET /api/accounts) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export function accountsStatus(config, statePath = DEFAULT_STATE_PATH, { now = Date.now(), secretsPath = DEFAULT_SECRETS_PATH } = {}) {
  // Read secrets too so `configured` reflects a pasted key/token â€” but only its PRESENCE is
  ...
  return Object.entries(reg).map(([provider, prov]) => {
    ...
    // optional per-provider usage caps (uses count) â†’ UI draws a % bar. config.providers.<p>.usage
    ...
    return {
      ...
      accounts: prov.accounts.map((a) => {
        ...
        // antigravity authenticates via the Antigravity IDE (Google keyring) â€” `agy -p` works with no
        ...
        return {
          ...
          // codex/claude have an interactive CLI login (dir) â†’ login button. antigravity uses the IDE keyring.
